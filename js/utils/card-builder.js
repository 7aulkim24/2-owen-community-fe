...
export function buildBadge(postType) {
    ...
    const modifier = `post-card__badge--${type}`;
    return `<span class="post-card__badge ${modifier}">${label}</span>`;
}
...
export function buildSourceSummary(sourceSummary) {
    ...
    const parts = [];
    if (sourceSummary.commits != null) parts.push(`커밋 ${sourceSummary.commits}건`);
    if (sourceSummary.pullRequests != null) parts.push(`PR ${sourceSummary.pullRequests}건`);
    if (sourceSummary.issues != null) parts.push(`이슈 ${sourceSummary.issues}건`);
    if (sourceSummary.repoName) parts.push(sourceSummary.repoName);
...
export function buildPostCard(post, getFullImageUrl) {
    ...
    return `
        <article class="post-card post-card--${postType}" onclick="location.href='/post-detail.html?id=${post.postId ?? post.post_id}'">
            ${badge}
